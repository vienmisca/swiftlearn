...
<body <body 
  x-data="{openEditModal: false, openDeleteModal: false, materiToEdit: {}, materiToDelete: {}}" style="background-image: url('{{ asset('images/bg-mentor.png') }}');" class="bg-cover bg-center min-h-screen p-8 relative overflow-auto">
  <!-- Header -->
      ...
      @forelse ($materis as $materi)
        <div class="bg-white rounded-2xl shadow flex items-center justify-between p-4">
          ...
          <div class="flex gap-2">
            <button
  @click.prevent="openEditModal = true; materiToEdit = {{ $materi->toJson() }}"
  class="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-full text-sm font-semibold"
>
  Edit
</button>
            <form method="POST" action="{{ route('mentor.delete.materi', $materi->id) }}">
              @csrf @method('DELETE')
              <button type="button"
        @click="openDeleteModal = true; materiToDelete = {{ $materi->toJson() }}"
        class="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-full text-sm font-semibold">
  Hapus
</button>
            </form>
          </div>
  ...
  <div @click.outside="openEditModal = false"
    ...
    <form :action="`/mentor/materi/${materiToEdit.id}`" method="POST" enctype="multipart/form-data">
      ...
      <!-- Judul -->
      <input type="text" name="judul" x-model="materiToEdit.judul"
             class="w-full mb-4 px-4 py-2 border rounded-xl"
             placeholder="Judul Materi" required>

      <!-- Deskripsi -->
      <textarea name="deskripsi" x-model="materiToEdit.deskripsi"
                class="w-full mb-4 px-4 py-2 border rounded-xl h-24 resize-none"
                placeholder="Deskripsi Materi"></textarea>

      <!-- Google Form Link -->
      <input type="url" name="google_form_link" x-model="materiToEdit.google_form_link"
             class="w-full mb-4 px-4 py-2 border rounded-xl"
             placeholder="Link Google Form">

      <!-- Optional: Upload PDF/Video -->
...
<div x-show="openDeleteModal"
  ...
  <div @click.outside="openDeleteModal = false"
    ...
    <p class="mb-6 text-gray-700">
      ...
      <span class="font-semibold" x-text="materiToDelete?.judul"></span>?
    </p>
